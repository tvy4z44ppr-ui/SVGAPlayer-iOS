...
static SVGAParserCacheKeyFilterBlock sharedCacheKeyFilter;
...
+ (void)setSharedCacheKeyFilter:(SVGAParserCacheKeyFilterBlock)cacheKeyFilter {
    @synchronized (self) {
        sharedCacheKeyFilter = [cacheKeyFilter copy];
    }
}

+ (SVGAParserCacheKeyFilterBlock)sharedCacheKeyFilter {
    @synchronized (self) {
        return sharedCacheKeyFilter;
    }
}
...
- (nonnull NSString *)cacheKey:(NSURL *)URL {
    NSString *keyString = URL.absoluteString ?: @"";
    NSString *scheme = URL.scheme.lowercaseString;
    // 仅对网络 URL 应用过滤器，避免影响 parseWithNamed: 传入的 file:// 本地资源
    ...
    return [self MD5String:keyString];
}
...
- (void)clearLocalSVGADataCacheWithURLString:(nonnull NSString *)URLString {
    ...
    NSString *cacheKeyMD5 = url != nil ? [self cacheKey:url] : [self MD5String:URLString];
    NSString *cacheDir = [self SVGADataCacheFilePath:cacheKeyMD5];
    NSError *error = nil;
...
- (void)downloadSVGADataWithURLString:(NSString *)URLString {
    [parseQueue addOperationWithBlock:^{
        ...
        NSString *cacheKeyMD5 = [self cacheKey:url];
        // 读取本地SVGAData缓存
